...
管理后台商户详情里已经有一个"合作伙伴"标签页，原本只是一个只读统计报表，背后是一个写死的三值枚举（`P01`=直连、`P02`=Meta Trader、`P03`=On Ramp），没有真正的渠道/合作伙伴数据结构，也没有任何返佣/佣金相关的代码——这是一个从零设计的新系统，只有"充值地址按来源分流"这一条底层机制可以直接复用（`SubWalletAddress` 按 `partnerCode` 区分，同一客户可以有多个地址）。原型最终把绑定+费率+返佣的配置直接做进了这个已有的"合作伙伴"标签页（见 §2），而不是新开一个平级菜单。

## 1. 核心概念

- **渠道（Channel）**：比如"Meta Trader"，是一类合作来源的统称，只记录名称、状态等基本信息，本身**不带费率**。
- **合作伙伴（Partner）**：挂在某个渠道下的具体个人/机构，记录名称、联系方式、状态，预留钱包地址字段（供未来自动打款用，本期不接真实打款）。渠道与合作伙伴的增删改**合并在同一个"渠道管理"页面里**——管理某个渠道的同时，就能管理这个渠道下挂的合作伙伴，不用在两个菜单之间来回切。这里只维护基本信息与"合作伙伴属于哪个渠道"的关联关系，不涉及费率或商户绑定。
- **商户的渠道配置（费率 + 返佣，在商户详情自己的「合作伙伴」标签页配置）**：为某个商户（broker）选择一个渠道，**并为这个商户单独录入该渠道下的充值/提现手续费率**（同一渠道下不同商户的费率可以不同，是商户与渠道谈好的结果，不是渠道的固定属性），再从这个渠道下已有的合作伙伴库里勾选若干个，为每个合作伙伴单独设置返佣比例（可挂几个不设上限，只卡"总和 ≤ 这笔手续费"）。
  - 之所以把费率从"渠道"移到"商户的渠道配置"：费率本质是商户与渠道谈判的结果，不同商户走同一个渠道也可能谈到不同的价格；渠道本身只是一个分类/口径，不应该替商户预设死一个费率。
  - 这个配置放在**商户详情自己的「合作伙伴」标签页**，而不是另开一个独立菜单：操作起点始终是"这个商户要怎么配"，商户详情页本身就已经是这个商户的上下文，不需要再单独选一次商户；也避免了跟"渠道管理"页面里同样叫"合作伙伴"的基础信息管理重名混淆。
...
| 商户详情 ›「合作伙伴」标签页（本系统新增） | 配置了渠道的商户，其渠道来源的客户 | 本系统新增的收费规则，按商户单独录入，替换基础费率，返佣从这笔手续费里出 |

三处顶部都应带一条一致的提示条，注明"系统收费规则分三处配置"，当前页高亮，另外两处可点击跳转，避免运营人员找错地方改错费率——第三段没有固定的下钻页面（需要先选到具体商户），链接落到商户列表，由使用者自己选商户后进入其详情页。
...
) COMMENT '商户的渠道配置（费率），在商户详情自己的「合作伙伴」标签页维护；下面的 partner_id/rebate_rate 挂在这条配置上';
